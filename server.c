...
int main()
{
	...
	pthread_create(&pt_ctrl, NULL, &controller, NULL);
...
void* controller(void *t)	//control the game's order.
{
	int i, n, j;
	struct msgbuf msg;
	...
	while(1)
	{
		...
		{
			printf("LOG: controller SEND \"TURN\" TO %d\n", connecter_fd[i]);
			...
			//Add pokers part, under consideration. Will add this function further.
